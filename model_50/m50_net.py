...
global_model_description = "conv(2/3/4/5/6/7/8/9/10x128)[l2(0.01)] -> relu -> maxpool(2) -> dropout(0.5) -> batchnorm -> lstm(512) -> dropout(0.5) -> merge(concat) -> dense(1)  [ 16 batch size ]"
...
def build_model(top_words, embedding_vecor_length, max_review_length, show_summaries=False):
	...
	branch_2.add(Conv1D(filters=128, kernel_size=2, padding='same', kernel_regularizer=l2(.01)))
	...
	branch_4.add(Conv1D(filters=128, kernel_size=4, padding='same', kernel_regularizer=l2(.01)))
	...
	branch_6.add(Conv1D(filters=128, kernel_size=6, padding='same', kernel_regularizer=l2(.01)))
	...
	branch_8.add(Conv1D(filters=128, kernel_size=8, padding='same', kernel_regularizer=l2(.01)))
	branch_8.add(Activation('relu'))
	branch_8.add(MaxPooling1D(pool_size=2))
	...
	branch_10.add(Conv1D(filters=128, kernel_size=10, padding='same', kernel_regularizer=l2(.01)))
	branch_10.add(Activation('relu'))
	branch_10.add(MaxPooling1D(pool_size=2))
